...
// 时间 o(n) 空间o(n)
/*
由于给出的数据范围-1000 <= nums[i] <= 1000其中是有负数的，区间和的变化并不是稳定递增的，此时之前一贯的滑动窗口的做法就不可行了
在这里我们需要借助另一个前缀和的概念
...
我们知道了当前位置的前缀和，又知道了区间和k，那么我们只需要判断在当前位置之前有几个位置的前缀和为当前位置的前缀和 - k的情况即可
所以在遍历的求前缀和的时候，只需在哈希表中统计到不同的前缀和出现了几次，本题思路计算解析完成了
*/

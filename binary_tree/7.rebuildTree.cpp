...
class Solution
{
   public:
    ...
    TreeNode* buildTreeIndex(vector<int>& preorder, int prestart, int preend, vector<int>& inorder, int instart,
                             int inend)
    {
        if (prestart >= preorder.size() || instart >= inorder.size() || prestart > preend || instart > inend)
            return nullptr;
